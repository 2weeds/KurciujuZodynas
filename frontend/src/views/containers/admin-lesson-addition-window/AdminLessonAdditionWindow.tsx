...
export const AdminLessonAdditionWindow = ({token}: Props) => {
        ...
        partsAdded.map((part, index) => (
          <>
          {
            ...
            <TableRow sx={{backgroundColor: '#C5E3FC'}} key={index + '-part'}>
              ...
              <TableCell className={styleClasses.btnTableCells} key={index + '-partEditBtn'}><EditIcon onClick={handlePartEditUnsetClick}></EditIcon></TableCell>
              <TableCell className={styleClasses.btnTableCells} key={index + '-partRmBtn'}><RemoveIcon className={styleClasses.removeBtn} onClick={() => handlePartRemoval(part, index)}></RemoveIcon></TableCell>
            </TableRow> :
            <TableRow key={index + '-part'}>
              <TableCell className={styleClasses.nameTableCells} key={index + '-partName'}>{part.name}</TableCell>
              <TableCell className={styleClasses.btnTableCells} key={index + '-partEditBtn'}><EditIcon onClick={() => handlePartEditClick(part, index)}></EditIcon></TableCell>
              <TableCell className={styleClasses.btnTableCells} key={index + '-partRmBtn'}><RemoveIcon className={styleClasses.removeBtn} onClick={() => handlePartRemoval(part, index)}></RemoveIcon></TableCell>
            </TableRow>
          }
          </>
  ...
  const renderGrammarOption = () => {
    if (partBeingEdited === undefined || !partBeingEdited.subTopics.has('grammar'))
      return (
        <StyledOption value="grammar">Gramatika</StyledOption>
      )
  }
    ...
    if (partBeingEdited !== undefined)
      return (
        <Box className={styleClasses.form}>
            ...
            <TextField value={partBeingEdited.name} error={partNameErrors !== undefined} className={styleClasses.inputField} variant="outlined" label="Pamokos dalies pavadinimas" size="small" multiline maxRows={2} onChange={handleLessonPartNameChange}></TextField>
                ...
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell className={styleClasses.btnTableCells}>
                      <AddIcon className={styleClasses.addLessonPartButton} onClick={() => setIsSubtopicBeingAdded(true)}></AddIcon>
                    </TableCell>
            ...
            <Button className={clsx(styleClasses.submitButton, styleClasses.addButton)} onClick={() => handleSave()}>Išsaugoti</Button>
            <Modal
  ...
  return (
    <Box className={styleClasses.container}>
          <Box className={clsx(styleClasses.form, styleClasses.leftForm)}>
              ...
              <Table className={styleClasses.subTopicsTable}>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell className={styleClasses.btnTableCells}>
                      <AddIcon className={styleClasses.addLessonPartButton} onClick={() => handlePartAddition()}></AddIcon>
                    </TableCell>
                    <TableCell className={styleClasses.btnTableCells}></TableCell>
                  </TableRow>
                </TableHead>
            ...
            <Button className={clsx(styleClasses.submitButton, styleClasses.addButton)} onClick={() => handleSubmit()}>Pridėti pamoką</Button>
          </Box>
